...
- The COPR project chroots are synced from the live `copr-cli list-chroots` output, filtered to chroots whose architecture appears in `packages.json`, then restricted to the Fedora allowlist in `chroot_include_distros` and filtered again by excluded distro IDs or distro prefixes from the global `chroot_exclude_distros` list.
- All tracked packages currently target `aarch64` and `x86_64` on Fedora 43, Fedora 44, and Fedora rawhide, excluding `alma-kitten+epel-10-*`, `almalinux-kitten-10-*`, `centos-stream+epel-next-8-*`, `centos-stream-8`, `custom-*`, `epel-7`, `fedora-eln-*`, `mageia-*`, `openeuler-*`, `rhel-7`, and `rhel-8`.
